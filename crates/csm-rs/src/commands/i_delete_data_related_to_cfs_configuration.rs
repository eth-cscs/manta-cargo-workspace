...
pub async fn exec(
    ...
    let cfs_configuration_name_from_bos_sessiontemplate_value_iter = bos_sessiontemplate_vec
        .iter()
        .map(|bos_sessiontemplate| bos_sessiontemplate.get_configuration().unwrap_or_default());
    ...
    let cfs_configuration_name_from_cfs_sessions = cfs_session_vec
        .iter()
        .map(|cfs_session| cfs_session.get_configuration_name().unwrap_or_default());
    ...
    let mut image_id_vec = cfs::session::utils::get_image_id_from_cfs_session_vec(&cfs_session_vec);
    ...
    for (cfs_configuration_name, mut image_id_vec) in cfs_configuration_image_id {
        let mut nodes_using_cfs_configuration_as_dessired_configuration_vec = cfs_component_vec
            .iter()
        ...
        for image_id in &image_id_vec {
            let node_vec = get_node_vec_booting_image(image_id, &bss_bootparameters_vec);
        ...
        || !image_id_used_to_boot_nodes_vec.is_empty()
    {
        ...
        image_id_vec
            .retain(|image_id| !image_id_used_to_boot_nodes_vec.contains(&image_id.as_str()));
